...
def submit_post(
    ...
    for area in _expertise_areas :
        ...
        x=fame_levels.filter(expertise_area=area['expertise_area'], fame_level__numeric_value__lte=0)
        if (len(x)>0):
...
def bullshitters():
    ...
    res = {}
    for area in ExpertiseAreas.objects.all():
        area_postivie_users = Fame.objects.filter(expertise_area=area, fame_level__numeric_value__lt=0)
        l = []
        #print(area_postivie_users)
        for positiv_user in area_postivie_users:
            dic = {}
